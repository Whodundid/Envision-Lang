...
public class PS_Method extends ParserStage {
	...
	//		       return Thing(other.value + value)
	//		   }
	//     }
	// }
	//
	...
	public static Statement methodDeclaration(ParserDeclaration declaration) { return methodDeclaration(false, declaration); }
	public static Statement methodDeclaration(boolean operator, ParserDeclaration declaration) {
	...
	private static Token getOperator() {
		if (match(BRACKET_LEFT)) {
			errorIf(!match(BRACKET_RIGHT), "Expected an operator!");
			return Token.create(Keyword.ARRAY_OPERATOR, "[]", current().line);
		}
		else if (match(NUMBER)) {
			//System.out.println("matching number keyword: " + previous().lexeme + current().lexeme + next().lexeme);
			return Token.create(Keyword.NUMBER, current());
		}
		else {
	...
	public static EArrayList<StatementParameter> getMethodParameters(boolean operator, String methodType) {
		...
		consume(EXPR_LEFT, "Expected '(' after method name!");
		...
		if (!check(EXPR_RIGHT)) {
			...
			if (operator) {
				...
				if ((checkType(DATATYPE) || check(IDENTIFIER)) && (checkNext(VARARGS) || !checkNext(COMMA, EXPR_RIGHT, ASSIGN))) {
					lastType = getAdvance();
				}
				...
				if (match(VARARGS)) { error("An operator method cannot take '...' varaiable arguments!"); }
				...
				if (matchType(ASSIGNMENT)) {
					assign = new AssignExpression(paramName, previous(), expression());
				}
				...
				do {
					...
					if ((checkType(DATATYPE) || check(IDENTIFIER, OPERATOR_)) && (checkNext(VARARGS) || !checkNext(COMMA, EXPR_RIGHT, ASSIGN))) {
						lastType = current();
						advance();
					}
		...
		consume(EXPR_RIGHT, "Expected ')' after parameters!");
		return parameters;
	}
	...
	public static EArrayList<Statement> getMethodBody(boolean constructor) {
		...
		if (!constructor) {
			if (match(LAMBDA)) {
				...
				body.add(ParserStage.returnStatement());
			}
			else if (match(SCOPE_LEFT)) {
				body = getBlock(true);
			}
			else {
			...
			else if (match(SCOPE_LEFT)) body = getBlock(true);
