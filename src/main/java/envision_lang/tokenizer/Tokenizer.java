...
public class Tokenizer {
     ...
     * <p>
     * Identifiers are used as names for variables, objects, etc. within
     * Envision.
     */
    private void identifier() {
        while (isLetterOrNum(peek()))
            advance();
    ...
    /**
     * Performs the same 'check' operation as check(char expected), but
     * requires that the given string of chars is present exactly as given.
    ...
    private char peekNext() {
        return (cur + 1 >= currentLineSource.length()) ? '\0' : currentLineSource.charAt(cur + 1);
    }
    ...
    private boolean tokenizeFile_I() throws IOException {
        try (var reader = new LineReader(theFile)) {
            while (reader.hasNextLine()) {
                final String line = reader.nextLine();
                final boolean hasNextLine = reader.hasNextLine();
    ...
    public boolean tokenizeLines(Iterable<String> linesToTokenize) {
        if (linesToTokenize == null) return false;
        
        // iterate through each line and parse out statements
        var it = linesToTokenize.iterator();
        
        while (it.hasNext()) {
            final String line = it.next();
            final boolean hasNextLine = it.hasNext();
    ...
    public boolean tokenizeLine(String lineIn) {
        ...
        for (int i = 0; i < subLines.length; i++, lineNum++) {
            final String line = subLines[i];
            final boolean hasNextLine = i < (subLines.length - 1);
