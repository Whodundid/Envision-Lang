...
public class EnvisionInterpreter implements StatementHandler, ExpressionHandler {
    ...
    /**
     * The initial (and primary) working directory for which all Envision code
     * executions will have originated from.
     */
    private static WorkingDirectory topDir;
    
    /**
     * Lowest scope level intended exclusively for language packages and
     * internal functions.
     */
    ...
    /**
     * The active working file directory for which this interpreter has been
     * created from. This working directory contains all of the related
     * codeFiles enclosed within the given directory.
     */
    ...
    /**
     * Keeps track and manages all user-defined datatypes during instance
     * creation for any object that is created within this interpreter.
     * <p>
     * A user-defined type is any datatype (class) that has been defined by
     * Envision code executed at runtime. This includes any types defined by
     * any internal or user-defined EnvisionPackages.
     */
    ...
    public EnvisionInterpreter(EnvisionProgram programIn) {
        this(programIn, programIn.getMainCodeFile());
    }
    
    public EnvisionInterpreter(EnvisionProgram programIn, EnvisionCodeFile startingFileIn) {
        program = programIn;
        startingFile = startingFileIn;
        fileName = startingFile.getFileName();
        
        WorkingDirectory dirIn = programIn.getWorkingDir();
        
        // it may be a bad idea to do this... too bad!
        internalScope.values().putAll(startingFile.scope().values());
    ...
    public static EnvisionInterpreter build(EnvisionProgram programIn, EList<String> userArgs) throws Exception {
        var interpreter = new EnvisionInterpreter(programIn);
    ...
    public static EnvisionInterpreter interpret(EnvisionProgram programIn, EnvisionCodeFile startingFileIn,
        EList<String> userArgs) throws Exception {
        var interpreter = new EnvisionInterpreter(programIn, startingFileIn);
        interpreter.setupWithUserArguments(userArgs);
    ...
    public void setUserArguments(EList<String> userArgs) {
    ...
    private void executeNext_i() {
        ...
        while (frameStatements.isNotEmpty()) {
            ...
            if (program.areBlockingStatementsEnabled() && (!runNext || s.isBlockingStatement())) {
     ...
     * a ParsedStatementError will be thrown instead.
     * 
    ...
    /**
     * Returns true if the given object is an EnvisionBoolean and the boolean
     * value is true.
     * 
     * @param obj
     * 
    ...
    /**
     * Returns the program that is ultimately running all code that this
     * interpreter is executing.
     * 
     * @return The root EnvisionProgram of this interpreter
     */
    public EnvisionProgram program() { return program; }
    
    /** Handles standard program console print statements. */
    public void printToConsoleReceiver(String toPrint, boolean newLine) {
        var r = program.getConsoleReceiver();
        if (r == null) {
            if (newLine) System.out.println(toPrint);
    ...
    /**
     * Returns the active working file directory for which this interpreter has
     * been created from. This working directory contains all of the related
     * codeFiles enclosed within the given directory.
     * 
    ...
    /**
     * Returns the initial (and primary) working directory for which all
     * Envision code executions will have originated from.
     * 
    ...
    /**
     * Returns the lowest scope level of this interpreter. This internal scope
     * contains all native Envision members that are defined upon this
     * interpreter's creation. Execute caution when modifiying the values
     * present within this scope as they cannot be restored to their original
     * state.
     * 
    ...
    /**
     * Returns the TypeManager for this interpreter. The type manager actively
     * keeps track of all user-defined class types that have been defined
     * within this interpreter's scope of execution. User-defined types that
     * are defined outside of this interpreter will need to be imported in
     ...
     * standard variable declaration process will be used. However, if the
     * value is currently defined, the existing value will be completely
     * overwritten, regardless of scope or visibility.
     * <p>
     * In terms of validity, it must be understood that this method will not
     * attempt to check for logical variable overwrites, I.E. forcefully
     * changing defined variable datatypes. Furthermore, this method will not
     * attempt to preserve already existing values under the same name as
     * existing values are completely overwritten. In terms of security, this
     * method should be used with the utmost amount of caution as declaration
     * overwrites will completely ignore existing variable modifiers, such as
     * private or final. This means that potentially sensitive data could be
     * overwritten regardless of scope and visibility. As such, this method
     * should be treated as a <u><strong>VERY DESTRUCTIVE</strong></u> method
     * of variable declaration which could have dramatic effects on further
     * program execution if used improperly.
     * <p>
     * In nearly every circumstance, the standard variable declaration
     * approaches should be preferred over this method.
     * 
     * @param name   The name of the object which will either be defined or
     *               overwritten
     * @param object The object being stored at the given 'name' location.
     * 
     ...
     * overwrites will completely ignore existing variable modifiers, such as
     * private or final. This means that potentially sensitive data could be
     * overwritten regardless of scope and visibility. As such, this method is
     * potentially a very destructive means of variable declaration, which
     * could have dramatic effects on further program execution if used
     * improperly.
     * <p>
     * In nearly every circumstance, the standard variable declaration
     * approaches should be preferred over this method.
    ...
    /**
     * This method will attempt to define a given object if it is found to not
     * already exist within the current interpreter scope. In the event that a
     * value under the same name is already defined, no further action will
     * take place and this method will execute quietly.
     * <p>
     * This method will not attempt to check for inconsistent datatypes between
     * existing and incomming values. Simply put, this method only checks if a
     * value of the same 'name' is currently defined within the current
     * interpreter scope or not. Because of this, it must be understood that
     * just because a value under the same name exists within the current
     * interpreter scope, it may not be the variable that was expected.
     * 
     ...
     * variable under the same name with the given object. In the event that
     * the object is not defined under the current interpreter scope, the given
     * object will be defined in its place.
     * <p>
     ...
     * method could potentially lead to destructive variable declarations such
     * that existing variables under the same name, but with a different
     ...
     * datatypes are consistent with already existing variable datatypes. In
     * the event that the incomming datatype does not match, or is not an
     * instance of (polymorphism), the existing variable datatype, an
     * InvalidDatatypeError is thrown. However, in the case that the incomming
     * object is 'NULL', then null (EnvisionNull) will be assigned to the
     * existing variable.
     * 
     * @param name   The name for which to define the given object on
     * @param object The object to be updated or defined if not already present
    ...
    public EnvisionObject updateOrDefine(String name, IDatatype type, EnvisionObject object) {
        ...
        if (!o.getDatatype().equals(type)) throw new InvalidDatatypeError("The incomming datatype: '" + type +
     ...
     * interpreter scope, then the variable is returned, otherwise Java:null is
     * returned instead.
     * 
